...
\nt{In this section we provide a proof of Theorem \ref{thm:distscoreschar} and state the analogous result for the inner confidence sets obtained using the distance transformation.
\begin{proof}
...
\nt{
\begin{theorem}\label{thm:distscoreschar2}
	For each $v \in \mathcal{V}$, let $f_I(s(X), v) = d_\rho(\hat{M}(X), v)$ and define $I(X)$ as in Section \ref{SS:MCS}. Suppose that $H(\hat{M}(X_i), Y_i) \leq k$, some $k \in \mathbb{R}$, for all $i \in J$, for some $J \subseteq \lbrace 1, \dots, n \rbrace$ such that $\frac{|J|}{n} > 1-\alpha_1$. Then $H(\hat{M}(X_{n+1}), I(X_{n+1}))  \leq k$.  In particular if $H(\hat{M}(X_{n+1}), Y_{n+1}) \leq k$, then $H(I(X_{n+1}), Y_{n+1})  \leq 2k$.
\end{theorem}
}

%\subsection{Confidence sets for multiple structures}
%Our results generalize to multiple structures. Suppose that we have 
...
\subsection{Writing the test time steps as an algorithm}

\nt{In order to clarify what is done at test time we include the following algorithm which demonstrates this for the polyps data application.} 

%\begin{algorithm}
%	\caption{Test time application of the methods (for the polyps data application)}\label{alg:bubble_sort}
%	\begin{algorithmic}[1]
%		\REQUIRE Inner and outer alpha levels $\alpha_1$ and $\alpha_2$ and thresholds $\lambda_I(\alpha_1)$ and $\lambda_O(\alpha_2)$ obtained as in equations \eqref{lambdaI} and \eqref{lambdaO} from the calibration dataset and test time observation $X_{n+1}$
%		\STATE Compute the predicted mask as $\hat{M}(X_{n+1}) = \lbrace v \in \mathcal{V}: s(X_{n+1}, v) > 0 \rbrace $
%		\STATE Calculate the set of points $E(\hat{M}(X_{n+1}))$ on the boundary of the predicted mask $\hat{M}(X_{n+1})$ using the marching squares algorithm.
%		\STATE Compute $ 
%		d_{\rho}(\hat{M}(X_{n+1}), v) = \text{sign}(\hat{M}(X_{n+1}), v)\min\lbrace \rho(v, e): e \in E(\hat{M}(X_{n+1}))\rbrace,$ i.e. the distance transformed scores, for each $v \in \mathcal{V}$.
%		\STATE Let $I(X_{n+1}) = \lbrace v \in \mathcal{V}: s(X, v) >\lambda_I(\alpha_1)  \rbrace $.
%		\STATE Let $O(X_{n+1}) = \lbrace v \in \mathcal{V}: -d_{\rho}(\hat{M}(X_{n+1}), v) \leq \lambda_O(\alpha_2)  \rbrace$.
%		\RETURN $I(X_{n+1}) $ and $O(X_{n+1}).$
%	\end{algorithmic}
%\end{algorithm}

\begin{algorithm}
	\caption{Test time application of the methods (for the polyps data application)}\label{alg}
	\begin{algorithmic}[1]
		\REQUIRE Inner and outer alpha levels $\alpha_1$ and $\alpha_2$ and thresholds $\lambda_I(\alpha_1)$ and $\lambda_O(\alpha_2)$ obtained as in equations \eqref{lambdaI} and \eqref{lambdaO} from the calibration dataset with $f_I$ the identity and $f_O$ the distance transformed scores.  A test time observation $X_{n+1}$ and a score function $s(X_{n+1})$ obtained by an image segmenting model and a distance metric $ \rho. $
		\STATE Compute the predicted mask as $\hat{M}(X_{n+1}) = \lbrace v \in \mathcal{V}: s(X_{n+1}, v) > 0 \rbrace $
		\STATE Calculate the set of points $E(\hat{M}(X_{n+1}))$ on the boundary of the predicted mask $\hat{M}(X_{n+1})$ using the marching squares algorithm.
		\STATE Compute $ 
			d_{\rho}(\hat{M}(X_{n+1}), v) = \text{sign}(\hat{M}(X_{n+1}), v)\min\lbrace \rho(v, e): e \in E(\hat{M}(X_{n+1}))\rbrace,$ i.e. the distance transformed scores, for each $v \in \mathcal{V}$.
...
\end{algorithm}
\nt{
For the brain imaging application, $\lambda_I(\alpha_1)$ and $\lambda_O(\alpha_2)$ are instead computed from the calibration dataset with both $f_I$ and $f_O$ being the distance transformed scores. Then line 4 of the algorithm at test time is replaced by:  Let $I(X_{n+1}) = \lbrace v \in \mathcal{V}: d_{\rho}(\hat{M}(X_{n+1}), v)>\lambda_I(\alpha_1)  \rbrace $. For the teeth segmentation the inner scores are analogously replaced but with scores smoothing using a kernel with FWHM 2 pixels.}
...
{\fontsize{11pt}{13pt}\selectfont \textbf{BB scores}}
	\begin{center}
		\includegraphics[width=0.22\textwidth]{../figures/learning/scores/772.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_surf/772.png}	\includegraphics[width=0.22\textwidth]{../figures/learning/dist_surf/772.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_surf/772.png}\\
		\includegraphics[width=0.22\textwidth]{../figures/learning/images/772.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_crs_marginal90/772.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_crs_marginal90/772.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_crs_marginal90/772.png}\\
		\vspace{0.5cm}
		\includegraphics[width=0.22\textwidth]{../figures/learning/scores/202.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_surf/202.png}	\includegraphics[width=0.22\textwidth]{../figures/learning/dist_surf/202.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_surf/202.png}\\
		\includegraphics[width=0.22\textwidth]{../figures/learning/images/202.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_crs_marginal90/202.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_crs_marginal90/202.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_crs_marginal90/202.png}\\
			\vspace{0.5cm}
		\includegraphics[width=0.22\textwidth]{../figures/learning/scores/54.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_surf/54.png}	\includegraphics[width=0.22\textwidth]{../figures/learning/dist_surf/54.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_surf/54.png}\\
		\includegraphics[width=0.22\textwidth]{../figures/learning/images/54.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_crs_marginal90/54.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_crs_marginal90/54.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_crs_marginal90/54.png}
	\end{center}
...
{\fontsize{11pt}{13pt}\selectfont \textbf{BB scores}}
	\begin{center}
		\includegraphics[width=0.22\textwidth]{../figures/learning/scores/183.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_surf/183.png}	\includegraphics[width=0.22\textwidth]{../figures/learning/dist_surf/183.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_surf/183.png}\\
		\includegraphics[width=0.22\textwidth]{../figures/learning/images/183.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_crs_marginal90/183.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_crs_marginal90/183.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_crs_marginal90/183.png}\\
		\vspace{0.5cm}
		\includegraphics[width=0.22\textwidth]{../figures/learning/scores/530.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_surf/530.png}	\includegraphics[width=0.22\textwidth]{../figures/learning/dist_surf/530.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_surf/530.png}\\
		\includegraphics[width=0.22\textwidth]{../figures/learning/images/530.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_crs_marginal90/530.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_crs_marginal90/530.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_crs_marginal90/530.png}\\
		\vspace{0.5cm}
		\includegraphics[width=0.22\textwidth]{../figures/learning/scores/754.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_surf/754.png}	\includegraphics[width=0.22\textwidth]{../figures/learning/dist_surf/754.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_surf/754.png}\\
		\includegraphics[width=0.22\textwidth]{../figures/learning/images/754.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/score_crs_marginal90/754.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_crs_marginal90/754.png}
		\includegraphics[width=0.22\textwidth]{../figures/learning/dist_bt_crs_marginal90/754.png}
	\end{center}
...
\begin{figure}[h!]
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{1\height}{\rotatebox{90}{\textbf{\shortstack{Image}}}}
	\end{subfigure}
	\hspace{0.05cm}
	...
	\\
			\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.3\height}{\rotatebox{90}{\textbf{\shortstack{Logit scores}}}}
	\end{subfigure}
	\hspace{0.05cm}
	\begin{subfigure}{0.18\textwidth}
		\centering
	...
	\\
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.5\height}{\rotatebox{90}{\textbf{\shortstack{BB scores}}}}
	\end{subfigure}
	\hspace{0.05cm}
		\begin{subfigure}{0.18\textwidth}
		\centering
...
\begin{figure}[h!]
	...
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
	\centering
	\raisebox{0.8\height}{\rotatebox{90}{\textbf{\shortstack{Combo}}}}
\end{subfigure}
\hspace{0.05cm}
	\begin{subfigure}{0.18\textwidth}
		\centering
...
\begin{figure}[h!] % 'r' for right, and setting the width
	...
	% Second row of images
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.5\height}{\rotatebox{90}{ \textbf{\shortstack{DT scores}}}}
	\end{subfigure}
	\hspace{0.1cm}
	\begin{subfigure}{0.16\textwidth}
		\centering
	...
	% Second row of images
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.37\height}{\rotatebox{90}{ \textbf{\shortstack{Logit scores}}}}
	\end{subfigure}
	\hspace{0.1cm}
	\begin{subfigure}{0.16\textwidth}
		\centering
...
\begin{figure}[h!] % 'r' for right, and setting the width
	...
	% Second row of images
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.4\height}{\rotatebox{90}{ \textbf{\shortstack{FWHM = 5}}}}
	\end{subfigure}
	\hspace{0.1cm}
	\begin{subfigure}{0.16\textwidth}
		\centering
	...
	% Second row of images
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.3\height}{\rotatebox{90}{ \textbf{\shortstack{FWHM = 10}}}}
	\end{subfigure}
	\hspace{0.1cm}
	\begin{subfigure}{0.16\textwidth}
		\centering
	...
	% Second row of images
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.3\height}{\rotatebox{90}{ \textbf{\shortstack{FWHM = 15}}}}
	\end{subfigure}
	\hspace{0.1cm}
	\begin{subfigure}{0.16\textwidth}
		\centering
	...
	\vspace{-0.35cm}
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.3\height}{\rotatebox{90}{ \textbf{\shortstack{FWHM = 20}}}}
	\end{subfigure}
		\hspace{0.1cm}
	% Second row of images
	\begin{subfigure}{0.16\textwidth}
	...
	\vspace{-0.35cm}
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.3\height}{\rotatebox{90}{ \textbf{\shortstack{FWHM = 25}}}}
	\end{subfigure}
	\hspace{0.1cm}
	% Second row of images
	\begin{subfigure}{0.16\textwidth}
...
\begin{figure}[h!]
	\begin{center}
		...
		%		\includegraphics[width=0.22\textwidth]{../figures/efficiency/inner_ratio.pdf}
		%		%		\quad\quad
		%		%		\hspace{-0.8cm}
		%		\includegraphics[width=0.22\textwidth]{../figures/efficiency/outer_ratio.pdf}
	\end{center}
	\caption{\nt{Coverage levels of the inner and outer sets averaged over 1000 validations for the original, distance transformed (DT) and smoothed scores (smoothed with a full width at half maximum of 20mm). The nominal rate is acheived in all settings considered.}}\label{fig:coverage2}
\end{figure}
\newpage
...
\begin{figure}[h!]
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{1.1\height}{\rotatebox{90}{\small \textbf{\shortstack{Image}}}}
	\end{subfigure}
	...
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.5\height}{\rotatebox{90}{\small \textbf{\shortstack{DT scores}}}}
	\end{subfigure}
	\begin{subfigure}{0.15\textwidth}
		\centering
	...
	\\
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.4\height}{\rotatebox{90}{\small \textbf{\shortstack{Logit scores}}}}
	\end{subfigure}
		\begin{subfigure}{0.15\textwidth}
		\centering
...
\begin{figure}[h!]
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.7\height}{\rotatebox{90}{\small \textbf{\shortstack{Original}}}}
	\end{subfigure}
	\begin{subfigure}{0.15\textwidth}
		\centering
		\includegraphics[width=\textwidth]{../figures/teeth/learning/orig_images/1_orig.png}
	...
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.4\height}{\rotatebox{90}{\small \textbf{\shortstack{FWHM = 2}}}}
	\end{subfigure}
	\begin{subfigure}{0.15\textwidth}
		\centering
	...
	\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.4\height}{\rotatebox{90}{\small \textbf{\shortstack{FWHM = 4}}}}
	\end{subfigure}
	\begin{subfigure}{0.15\textwidth}
		\centering
...
%	\end{subfigure}
	...
	\\
		\begin{subfigure}[b]{0.03\textwidth} % Adjust the width to fit your label
		\centering
		\raisebox{0.4\height}{\rotatebox{90}{\small \textbf{\shortstack{FWHM = 8}}}}
	\end{subfigure}
	\begin{subfigure}{0.15\textwidth}
		\centering
...
\begin{figure}[h!]
	...
	\begin{subfigure}{0.15\textwidth}
		...
		\label{fig:1}
	\end{subfigure}
	\caption{\nt{Inner and outer confidence sets for brain mask segmentation computed using the logit scores. The performance is less good than the score transformations optimized on the learning dataset and shown in the main text but have been included for reference. The outer sets are larger and less precise than those based on the distance transformation. The inner sets are good but not quite as good as the ones based on a small amount of smoothing.}}\label{fig:teethorig}
\end{figure}
...
\begin{figure}[h!]
	\begin{center}
		...
		%		\includegraphics[width=0.22\textwidth]{../figures/efficiency/outer_ratio.pdf}
	\end{center}
	\caption{\nt{Coverage levels of the inner and outer sets averaged over 1000 validations for the original, distance transformed (DT) and smoothed scores (smoothed with a full width at half maximum of 2 pixels). The nominal rate is acheived in all settings considered.}}\label{fig:coverage3}
\end{figure}
...
\begin{table}[h!]
	\nt{
	\centering
	\caption{\nt{Performance Metrics over the validation set}}
	\begin{tabular}{@{}lcccc@{}}
		\toprule
...
\subsection{Relationship with multiple testing error rates}\label{coveragerates}
